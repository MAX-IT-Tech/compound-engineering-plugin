...
**Mutual exclusion on option 4.** When `OUTPUT_FORMAT=html` for this run, option 4 is rendered as "Open in browser" and routes to local file opening (see routing below); when `OUTPUT_FORMAT=md`, option 4 stays "Open in Proof" with the existing HITL routing. Proof and local-browser review serve overlapping purposes, so the mutual exclusion keeps the menu within its 5-option cap. `/ce-work` remains the recommended option in both modes — `ce-work` consumes the markdown plan that always exists, and HTML is treated as a richer review/share surface, not a review gate.
