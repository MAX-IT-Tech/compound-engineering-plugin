...
**Mutual exclusion on option 4.** Option 4 has two mutually exclusive labels: render exactly one based on `OUTPUT_FORMAT`. Proof operates on markdown plans (it ingests the `.md` source and rewrites markdown), so it does not work as a review surface for HTML output — HTML-mode users see "Open in browser" instead. The single-label rendering avoids a confusing "show both" outcome, keeps the menu within its 5-option cap, and prevents an HTML-mode user from picking Proof and getting markdown loaded in the editor unexpectedly. `/ce-work` remains the recommended option in both modes — `ce-work` consumes the markdown plan that always exists, and HTML is a richer review/share surface, not a review gate.
