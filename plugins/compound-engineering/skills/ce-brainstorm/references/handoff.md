...
3. **Open in Proof — review and comment to iterate with the agent** - Open the doc in Every's Proof editor, iterate with the agent via comments, or copy a link to share with others. Shown only when a requirements document exists. **Render this label only when `OUTPUT_FORMAT=md`.**
3. **Open in browser** — open the `.html` sibling locally for review and sharing. **Render this label only when `OUTPUT_FORMAT=html`** (replaces "Open in Proof" at the same slot). Proof operates on markdown documents and cannot ingest HTML, so HTML-mode users see the local-browser option instead. Shown only when a requirements document exists.
